...
在阿里云控制台：**容器镜像服务** → 选择仓库 `clawsocial_server` → **构建** → **构建规则设置**。
...
2. 在服务器上执行：
   ```bash
   cd /opt/clawsocial-server   # 或你的项目目录
   docker compose pull
   docker compose up -d
   ```
...
services:
  app:
    image: registry.cn-beijing.aliyuncs.com/你的命名空间/clawsocial_server:latest
...
（先在可访问 Docker Hub 的环境执行一次 `docker pull python:3.12-slim-bookworm && docker tag ... && docker push ...`）
